...
-(NSMutableDictionary*) dictionary{
    ...
    if([self notOperator])
    {
        [result setValue:[self notOperator] forKey:@"not"];
    }
    if([self range])
    {
        NSMutableDictionary<NSString*,NSMutableDictionary*>* value=[[NSMutableDictionary<NSString*,NSMutableDictionary*> alloc] init];
        for(NSString* rangeKey in self.range){
            Range* rangeValue = [[self range]valueForKey:rangeKey];
            if (rangeValue){
                NSDictionary* rangeDictionary = [rangeValue dictionary];
                [value setValue:rangeDictionary forKey:rangeKey];
            }
        }
        [result setValue:value forKey:@"range"];
//  [result setValue:[[self range] dictionary]forKey:@"range"];
    }
    if([self regexp])
    {
    ...
    {
        NSMutableDictionary<NSString*,NSMutableDictionary*>* value=[[NSMutableDictionary<NSString*,NSMutableDictionary*> alloc] init];
        for(NSString* simpleQueryKey in self.simpleQuery){
            SimpleQuery* simpleQueryValue = [[self range]valueForKey:simpleQueryKey];
            if (simpleQueryValue){
                NSDictionary* simpleQueryDictionary = [simpleQueryValue dictionary];
                [value setValue:simpleQueryDictionary forKey:simpleQueryKey];
            }
        }
        [result setValue:value forKey:@"simpleQuery"];
     //   [result setValue:[[self simpleQuery] dictionary]forKey:@"simpleQuery"];
    }
    if([self substring])
    {
...
-(id) initWithDic:(NSDictionary*)dictionary{
    ...
    self.notOperator = [dictionary objectForKey:@"not"];
    
  //  self.range = [[NSMutableDictionary<NSString*,Range*> alloc]initWithDic:[dictionary objectForKey:@"range"]];
    self.regexp = [dictionary objectForKey:@"regexp"];
    self.simple_query_string = [dictionary objectForKey:@"simple_query_string"];
 //   self.simpleQuery = [[NSMutableDictionary<NSString*,SimpleQuery*> alloc]initWithDic:[dictionary objectForKey:@"simpleQuery"]];
